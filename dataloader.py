...
class CTCaseDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        series_uid = pd_row.SeriesInstanceUID

        # ---- Try .npy path first (LUNA25 default)
        ...
        if image_path.exists() and metadata_path.exists():
            ...
            # ---- Otherwise, fallback to .mha file ----
            mha_path = self.data_dir / "volume" / f"{series_uid}.mha"
            if not mha_path.exists():
                raise FileNotFoundError(f"Missing file for {series_uid}: {mha_path}")

            image = sitk.ReadImage(str(mha_path))
            img = sitk.GetArrayFromImage(image).astype(np.float32)  # [D,H,W]
            spacing = np.array(image.GetSpacing())[::-1]  # sitk uses (x,y,z)
            origin = np.array(image.GetOrigin())[::-1]
            transform = np.eye(3)

        # ---- Standard LUNA25 patch extraction ----
        if self.mode == "2D":
